# primary energy ----------------------------------------------------------
prmene <- tribble(~Sv,~Legend,~Color,
                  ...
                  'Prm_Ene_Hyd','Hydrogen','lightsteelblue',
...
# Secondary energy --------------------------------------------------------
secele <- tribble(~Sv,~Legend,~Color,
...
sechyd <- tribble(~Sv,~Legend,~Color,
...
# final energy ------------------------------------------------------------
finene <- tribble(~Sv,~Legend,~Color,
...
finind <- tribble(~Sv,~Legend,~Color,
...
finbui <- tribble(~Sv,~Legend,~Color,
...
finres <- tribble(~Sv,~Legend,~Color,
...
fincom <- tribble(~Sv,~Legend,~Color,
...
fintra <- tribble(~Sv,~Legend,~Color,
...
finpss <- tribble(~Sv,~Legend,~Color,
...
finfre <- tribble(~Sv,~Legend,~Color,
...
finroa <- tribble(~Sv,~Legend,~Color,
...
finavi <- tribble(~Sv,~Legend,~Color,
...
finshi <- tribble(~Sv,~Legend,~Color,
...
# final energy- Rail
finrai <- tribble(~Sv,~Legend,~Color,
...
# Sectoral emission
emisec <- tribble(~Sv,~Legend,~Color,
                  ...
                  'Emi_CO2_Ene_Dem_Oth_Sec','Other','grey',
                  'Emi_CO2_Ene_Dem_AFO','AFOFI','paleturquoise3',
                  'Emi_CO2_Ind_Pro','Industrial\nprocess','sandybrown',
                  'Emi_CO2_Oth','DACCS','thistle2')
                      
# Carbon capture
carcap <- tribble(~Sv,~Legend,~Color,
                  'Car_Cap_Ind_Pro','Industrial process','grey',
...
carseq <- tribble(~Sv,~Legend,~Color,
...
invadd <- tribble(~Sv,~Legend,~Color,
                  'Inv_Add_Ene_Dem_Ind','Industry','salmon',
                  'Inv_Add_Ene_Dem_Res_and_Com','buildings','lightsteelblue',
                  'Inv_Add_Ene_Dem_Tra','Transport','darkolivegreen2',
                  'Inv_Add_Ene_Sup','Energy supply','moccasin')
