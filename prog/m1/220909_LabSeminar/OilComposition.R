...
df_oil <- df_all %>% 
  ...
  filter(Year>=2020) %>%
  rename("totvar"=Value) %>% 
  filter(Scenario%in%c("Baseline","SR15_LOS"))

df_vx1 <- rgdx.param(paste(cdir,"data",date,project,"check","Baseline.gdx",sep="/"),"vx_l") %>%
  filter(L%in%c("OIL_CTL","OIL_CTLX","OIL_GTL","OIL_GTLX","OIL_WOL_OILM1","OIL_WOLX_OILM1")) %>% 
  mutate(Scenario="Baseline")
# df_vx2 <- rgdx.param(paste(cdir,"data",date,project,"gdx_primary","SR15_HOS.gdx",sep="/"),"vx_l") %>% 
#   filter(L%in%c("OIL_CTL","OIL_CTLX","OIL_GTL","OIL_GTLX","OIL_WOL_OILM1","OIL_WOLX_OILM1")) %>% 
#   mutate(Scenario="SR15_HOS")
#   df_vx3 <- rgdx.param(paste(cdir,"data",date,project,"check","SR15_LOS.gdx",sep="/"),"vx_l") %>%
  # filter(L%in%c("OIL_CTL","OIL_CTLX","OIL_GTL","OIL_GTLX","OIL_WOL_OILM1","OIL_WOLX_OILM1")) %>%
  # mutate(Scenario="SR15_LOS")

vx_l <- df_vx1
# vx_l <- bind_rows(df_vx1,df_vx3)
# vx_l <- bind_rows(df_vx1,df_vx2,df_vx3)

df_a1 <- rgdx.param(paste(cdir,"data",date,project,"check","Baseline.gdx",sep="/"),"a_t") %>% 
  filter(L%in%c("OIL_CTL","OIL_CTLX","OIL_GTL","OIL_GTLX","OIL_WOL_OILM1","OIL_WOLX_OILM1")) %>% 
  mutate(Scenario="Baseline")
# df_a2 <- rgdx.param(paste(cdir,"data",date,project,"gdx_primary","SR15_HOS.gdx",sep="/"),"a_t") %>% 
#   filter(L%in%c("OIL_CTL","OIL_CTLX","OIL_GTL","OIL_GTLX","OIL_WOL_OILM1","OIL_WOLX_OILM1")) %>% 
#   mutate(Scenario="SR15_HOS")
# df_a3 <- rgdx.param(paste(cdir,"data",date,project,"check","SR15_LOS.gdx",sep="/"),"a_t") %>%
#   filter(L%in%c("OIL_CTL","OIL_CTLX","OIL_GTL","OIL_GTLX","OIL_WOL_OILM1","OIL_WOLX_OILM1")) %>%
#   mutate(Scenario="SR15_LOS")

a_t <- df_a1
# a_t <- bind_rows(df_a1,df_a3)
...
plot(g)
ggsave(paste0(odir,"/ppt/CTLGTLarea.png"),g,width=10,height=6)
...
g <- df3 %>% 
  ...
  labs(y="Share in Oil Product Production (%)") +
  MyThemeLine
plot(g)
ggsave(paste0(odir,"/ppt/CTLGTLbar.png"),g,width=6,height=4)
